...
short command line could do exactly that.

None of the four are exotic. The worst is seven bytes.

## What is changed

Two commits on `ferentin-patches`, both confined to `brush-parser`:
...
- `cargo test -p brush-parser`: 231 passed, 0 failed. That is upstream's 224 plus
  7 regression tests added here.
...
- `cargo test -p brush-shell --test brush-compat-tests`: 1388 succeeded, 405
  failed, 366 known to fail, 43 skipped. Identical to a pristine checkout, so
  these changes regress nothing. The 405 failures are pre-existing and mostly
  reflect the local bash 3.2.
...
These are upstream bugs and belong upstream. Both commits are self-contained and
were written to be cherry-picked onto a fork of `reubeno/brush` and sent as pull
requests, which is the intended end state for this mirror: once the fixes land
upstream and a release carries them, a rebase drops our commits and the mirror
stops being needed.

None of the four had an issue filed at the time of the spike.
